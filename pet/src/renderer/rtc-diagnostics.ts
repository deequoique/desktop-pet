...
export async function collectRtcStats(pc: RTCPeerConnection) {
  ...
  stats.forEach((report: any) => {
    if (report.type === 'local-candidate' || report.type === 'remote-candidate') candidates.set(report.id, statsCandidate(report));
  ...
  return {
    ...
    pairs: pairs.sort((a, b) => Number(b.selected) - Number(a.selected)).slice(0, 40),
  };
}

export function attachRtcDiagnostics(pc: RTCPeerConnection, options: AttachOptions): RtcDiagnosticHandle {
  const base = () => ({ callId: options.getCallId() || undefined, role: options.role, mediaKind: options.mediaKind });
  const recordState = (stateKind: string, value: string) => {
    options.recorder({
      event: 'webrtc.state',
      domain: 'webrtc',
      level: value === 'failed' ? 'error' : value === 'disconnected' ? 'warn' : 'info',
      ...(value === 'failed' ? { errorCode: 'webrtc_ice_connectivity_failed', recoverability: 'retryable' as const }
        : value === 'disconnected' ? { errorCode: 'webrtc_ice_disconnected', recoverability: 'automatic' as const } : {}),
  ...
  const onIceConnection = () => {
    ...
    if (['connected', 'completed', 'failed', 'disconnected'].includes(pc.iceConnectionState)) void snapshot(`ice-${pc.iceConnectionState}`);
  };
  const onConnection = () => {
    recordState('connectionState', pc.connectionState);
    if (['connected', 'failed', 'disconnected', 'closed'].includes(pc.connectionState)) void snapshot(`connection-${pc.connectionState}`);
  };
  ...
  const onCandidateError = (rawEvent: Event) => {
    ...
    options.recorder({
      ...
      correlation: { callId: options.getCallId() || undefined },
      context: { ...base(), url: event.url, code: event.errorCode, text: event.errorText, address: event.address, port: event.port },
    });
  };
  ...
  return {
    candidate(direction, action, candidate, error) {
      options.recorder({
        ...
        ...(action === 'add-failed' ? { errorCode: 'webrtc_add_ice_candidate_failed', recoverability: 'retryable' as const } : {}),
        correlation: { callId: options.getCallId() || undefined },
        context: {
          ...base(),
          direction,
    ...
    close(reason) {
      ...
      options.recorder({
        ...
        domain: 'webrtc',
        correlation: { callId: options.getCallId() || undefined },
        context: { ...base(), reason, connectionState: pc.connectionState, iceConnectionState: pc.iceConnectionState, signalingState: pc.signalingState },
      });
