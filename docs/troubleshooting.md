...
- TRTC 新通话中的“我的麦克风”“对方麦克风”“对方系统声音”默认全部关闭，需要在本机逐项打开；共享方没有并不存在的“对方系统声音”开关。
- Windows 11 共享系统声音时会显示“已排除本应用/TRTC 的播放”。如果 helper 缺失或异常，只会停系统声音，屏幕和双向麦克风继续；导出的本地诊断包含 helper 协议、启动/停止/错误和丢帧计数，不包含 PID、路径、设备名或 PCM。
- Windows 10 继续使用整机系统回环，界面会提示当前 build 不支持数字回环排除；系统声音仍可共享，但远端语音可能被回采形成数字回声。升级 Windows 11 才能启用进程树排除，无需选择游戏或 exe。
- ICE 为 `failed`：当前 NAT 无法点对点打洞；需要配置可用 TURN。`relay` 时应用把屏幕限制为 640×360/5fps/240 kbit/s、每路摄像头限制为 320×180/10fps/120 kbit/s；profile 应用失败时只停对应视频并保留音频。credential、端口和带宽诊断见 [Ubuntu 24.04 coturn 部署手册](./ubuntu-coturn-deployment.md)。
