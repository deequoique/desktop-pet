...
托管供应商由 server 的 `TTS_PROVIDER` 选择。ElevenLabs 使用上游 HTTP MP3 流；CosyVoice 使用 DashScope WebSocket 的 `run-task`、`continue-task`、`finish-task` 工作流并接收 binary MP3 帧。二者最终都通过相同的一次性 HTTP job URL 交给 pet，因此队列、限流、播放和口型逻辑保持一致。CosyVoice 当前不提供客户端 BYOK。
